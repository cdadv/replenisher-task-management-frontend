...
export class CrudNgxTableComponent implements OnInit, OnDestroy {
    ...
    _.forEach(result, (task) => {
      ...
      _.forEach(task, (value, key) => {
        if (key == 'managerUserDTOSet') {
          ...
          taskProcessed.estimatedDuration = Math.floor(task[key] / (24 * 60 * 60 * 1000));
        } else {
  ...
  openPopUp(data: any = {}, isNew?) {
    let title = isNew ? 'Add new Task Template' : 'Update Task Template';
    ...
    dialogRef.afterClosed()
      .subscribe(res => {
        ...
        if (isNew) {
          ...
          this.crudService.updateItem(data.taskTemplateId, res)
            .subscribe(data => {
              this.getItemSub = this.crudService.getItems()
              .subscribe(newdata => {
                this.items = this.onData(newdata.result);    
  ...
  deleteItem(row) {
    this.confirmService.confirm({message: `Delete ${row.name}?`})
      .subscribe(res => {
        if (res) {
          this.loader.open();
          this.crudService.removeItem(row.taskTemplateId)
            .subscribe(data => {
              this.getItemSub = this.crudService.getItems()
              .subscribe(newdata => {
                this.items = this.onData(newdata.result);    
